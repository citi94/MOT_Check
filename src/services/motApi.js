...
export const getMotHistory = async (registration) => {
  try {
    // Use /api/ path for consistency with redirects
    const response = await fetch(`/api/getMotHistory?registration=${registration}`);
...
export const enableNotification = async (registration) => {
  try {
    // Use /api/ path for consistency with redirects
    const response = await fetch('/api/enableNotification', {
      method: 'POST',
...
export const disableNotification = async (registration) => {
  try {
    // Use /api/ path for consistency with redirects
    const response = await fetch('/api/disableNotification', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ registration })
    });
